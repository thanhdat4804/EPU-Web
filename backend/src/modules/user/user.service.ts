...
export class UserService {
  ...
  // CẬP NHẬT COUNTRY + ADDRESS
  async updateAddress(userId: number, data: { country?: string; address?: string }) {
  ...
  // THÊM / CẬP NHẬT ĐỊA CHỈ (UPSERT)
  async addAddress(userId: number, data: { country: string; address: string }) {
    return this.prisma.user.update({
      where: { id: userId },
      data: {
        country: data.country || 'Vietnam',
